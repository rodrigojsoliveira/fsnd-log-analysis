...
def getMostPopularAuthors():
    """Retrieves a list with the most popular authors of all time."""
    db = psycopg2.connect("dbname=news")
    cur = db.cursor()
...
def getErrorDay():
    """Retrieves the day(s) where there were more than 1% request errors."""
    db = psycopg2.connect("dbname=news")
    cur = db.cursor()
    cur.execute("""select the_day, result from
...
def getMonthString(monthNumber):
    ...
    return switcher.get(monthNumber, "nothing")
